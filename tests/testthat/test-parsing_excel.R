...
test_that("extract_all_desezoniranje_data integrates single file workflow", {
  ...
  file_paths <- c(test_single = test_file)
  result <- extract_all_desezoniranje_data(file_paths, config = mock_config)
...
test_that("extract_all_desezoniranje_data joins multi-source files correctly", {
  ...
  file_paths <- c(
    multi_source_a = test_file_a,
    multi_source_b = test_file_b
  )
  result <- extract_all_desezoniranje_data(file_paths, config = mock_config)
